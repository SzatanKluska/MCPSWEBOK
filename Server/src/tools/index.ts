 ...
 *
 * Exposes `swebok_search`: a retrieval-only RAG search that returns the most
 * relevant SWEBOK passages with citations. The calling LLM synthesizes the
 * final answer from them. Depends on a ready {@link Retriever} (index built at
 * startup), injected here.
 */
export function registerTools(server: McpServer, retriever: Retriever): void {
  server.registerTool(
    "swebok_search",
    {
      title: "SWEBOK Search",
    ...
    async ({ query, topK }) => {
      const k = topK ?? retriever.defaultTopK;
